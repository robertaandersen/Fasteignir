...
resource "aws_lb_target_group" "target_group_ip_80" {
...
resource "aws_lb_listener" "lb_port_80_listener" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.target_group_ip_80.arn
  }
}
resource "aws_lb_listener" "lb_port_8080_listener" {
  load_balancer_arn = module.alb.load_balancer_arn
  port              = "8080"
  protocol          = "HTTP"
